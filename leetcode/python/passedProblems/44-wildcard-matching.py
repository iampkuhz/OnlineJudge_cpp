...
class Solution(object):
    def isMatch(self, s,p):
            ...
            si,pi=0,0;ss=-1;
            while si < ls:
                if pi < lp and (s[si] == p[pi] or p[pi]=='?'):
                    si+=1;pi+=1;
                elif pi<lp and p[pi]=='*':
                    star=pi;pi+=1;ss=si;
                elif star>=0:
                    ss+=1;si=ss;pi=star+1;
                else: return False;
            while pi < lp and p[pi] == '*':
                pi += 1;
            return True if pi == lp else False;
...
#            elif si<0 or pi <0: return False;
#            re=False;
#            if(p[pi] == '?' or p[pi] == s[si]):
#                return find(si-1,pi-1);
#            elif p[pi] == '*':
#                return (find(si-1,pi-1) or find(si-1,pi) or find(si,pi-1));
#            return False;
#        re=find(len(s)-1,len(p)-1);
#        return re;
...
#    def isMatch(self, s, p):
#        import re;
#        pt='^' + p.replace('.','(\.)').replace('*','.*').replace('?','.') + "$";
#        #print p,pt,s
#        return bool(re.match(pt,s));
#

# 最后一个大样例超时
#class Solution(object):
#    def isMatch(self, s,p):
#        m=[[False for i in range(len(p)+1)] for i in range(2)];
#        m[1][0] = True;
#        j=1;
...
#                    m[i%2][j+1] = m[(i+1)%2][j];
#                elif p[j] == '*':
#                    m[i%2][j+1] = m[i%2][j] or m[(i+1)%2][j+1] or m[(i+1)%2][j];
#                else: m[i%2][j+1] = False;
#        return m[(len(s)+1)%2][len(p)];
